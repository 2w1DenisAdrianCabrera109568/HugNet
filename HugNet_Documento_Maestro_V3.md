...
\# HugNet — Documento Maestro (v3 - Sprint 3 finalizado)
...
12.1. Tarea 1: Iniciar donation-service (HU-14 y HU-15)
Modificar Donation.java: Añadir los campos payment_gateway_id (String) y payment_status (Enum) que ya estaban definidos en el plan.

Definir PaymentStatus Enum: Crear el Enum (PENDIENTE_PAGO, APROBADA, RECHAZADA).

Integración MercadoPago (Sandbox): Crear la lógica en DonationService para conectarse a la API de MercadoPago Sandbox y generar una "Preferencia de Pago" (link de pago).

Endpoint de Creación: Crear (o modificar) el endpoint POST /api/donations para que maneje DonationType.MONETARIA, cree la donación en PENDIENTE_PAGO y devuelva el link de MercadoPago.

Endpoint de Webhook: Crear el endpoint POST /api/donations/webhook/mp para recibir las notificaciones de MercadoPago y actualizar el estado de la donación a APROBADA.

12.2. Tarea 2: report-service (HU-20)
Implementar la lógica (vía WebClient) para generar el reporte de participación por tipo de evento.

12.3. Tarea 3: sponsor-service (HU-28)
Implementar la lógica para asignar Sponsors a un Activity. (Esto requerirá una tabla intermedia activity_sponsors y comunicación entre servicios).